...
func (g *GoGenerator) generateGroup(group *parser.ConstantGroup, projectLabel string) string {
	...
	code.WriteString("\n")
	code.WriteString(g.generateGetDescription(group, structName))
...
func (g *GoGenerator) generateFormat(group *parser.ConstantGroup, structName string) string {
	...
	code.WriteString(fmt.Sprintf("// Format 根据值和语言格式化%s的标签\n", groupName))
	code.WriteString(fmt.Sprintf("func (s %s) Format(value %s, lang string) string {\n", structName, valueType))
	code.WriteString(fmt.Sprintf("\tlabels := map[string]map[%s]string{\n", valueType))
	...
	code.WriteString("\t}\n\n")
	code.WriteString("\tif langMap, exists := labels[lang]; exists {\n")
	code.WriteString("\t\tif label, exists := langMap[value]; exists {\n")
	code.WriteString("\t\t\treturn label\n")
	code.WriteString("\t\t}\n")
	code.WriteString("\t}\n\n")
	code.WriteString("\t// 默认返回英文\n")
	code.WriteString("\tif enMap, exists := labels[\"en\"]; exists {\n")
	code.WriteString("\t\tif label, exists := enMap[value]; exists {\n")
	code.WriteString("\t\t\treturn label\n")
	code.WriteString("\t\t}\n")
	code.WriteString("\t}\n\n")
...
func (g *GoGenerator) generateGetDescription(group *parser.ConstantGroup, structName string) string {
	var code strings.Builder
	
	valueType := parser.GetGoType(group.Constants[0].Type)
	
	code.WriteString("// GetDescription 获取常量值的详细描述\n")
	code.WriteString(fmt.Sprintf("func (s %s) GetDescription(value %s) string {\n", structName, valueType))
	code.WriteString(fmt.Sprintf("\tdescriptions := map[%s]string{\n", valueType))
	...
	code.WriteString("\t}\n\t\n")
	code.WriteString("\tif desc, exists := descriptions[value]; exists {\n")
	code.WriteString("\t\treturn desc\n")
	code.WriteString("\t}\n")
	code.WriteString("\treturn fmt.Sprintf(\"未知常量值: %v\", value)\n")
	code.WriteString("}\n")
